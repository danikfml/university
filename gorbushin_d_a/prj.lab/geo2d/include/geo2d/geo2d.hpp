...
Style::Color::Color(int &r, int &g, int &b, double &a)
        : R(r), G(g), B(b), A(a) {
}

void Style::Color::setRGBA(int &r, int &g, int &b, double &a) {
    R = r;
    G = g;
    B = b;
    A = a;
...
namespace geometry {
    ...
    public:
        ...
        explicit Point(const Coordinates &pt, const Style::Color &color);

        void WriteTo(std::ofstream &ostream) const override;
    ...
    Point::Point(const Coordinates &pt, const Style::Color &color)
...
void geometry::Point::WriteTo(std::ofstream &ostream) const {
    ostream << "\\fill[" << color_.R << "](" << std::get<0>(point_) << ", " << std::get<1>(point_) << ") circle(2pt);"
            << std::endl;
}
...
void geometry::Elipse::WriteTo(std::ofstream &ostream) const {
    ostream << "\\draw (" << std::get<0>(center_) << ", " << std::get<1>(center_) << ") ellipse (" << halfA_ << " and "
            << halfB_ << ");" << std::endl;
}
...
void Style::Width::setWidth(double &num) {
    width_ = num;
}
...
void Style::Canvas::WriteTo(std::ofstream &os) const {
    ...
    os << "\t\\draw[help lines,use as bounding box] (" << std::get<0>(startPoint_) << "," << std::get<1>(startPoint_)
       << ") (" << std::get<0>(endPoint_) << "," << std::get<1>(endPoint_) << ");" << std::endl;
